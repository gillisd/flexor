RSpec.describe Flexor do
  describe "#merge" do
    ...
    it "deep merges nested hashes, preserving untouched values" do
      a = described_class.new({ db: { host: "localhost", port: 5432 } })
      b = a.merge({ db: { port: 3306, name: "mydb" } })
      expect(b.db.host).to eq "localhost"
    end

    it "deep merges nested hashes, updating changed values" do
      a = described_class.new({ db: { host: "localhost", port: 5432 } })
      b = a.merge({ db: { port: 3306, name: "mydb" } })
    ...
    it "deep merges multiple levels, preserving untouched values" do
      a = described_class.new({ a: { b: { c: 1, d: 2 } } })
      b = a.merge({ a: { b: { d: 3, e: 4 } } })
      expect(b.a.b.c).to eq 1
    end

    it "deep merges multiple levels, updating changed values" do
      a = described_class.new({ a: { b: { c: 1, d: 2 } } })
      b = a.merge({ a: { b: { d: 3, e: 4 } } })
