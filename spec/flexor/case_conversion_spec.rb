RSpec.describe Flexor::CaseConversion do
  ...
  describe ".case_counterpart" do
    it "returns camelCase symbol for snake_case input" do
      expect(described_class.case_counterpart(:foo_bar)).to eq :fooBar
    end

    it "returns snake_case symbol for camelCase input" do
      expect(described_class.case_counterpart(:fooBar)).to eq :foo_bar
    end

    it "returns nil for lowercase-only keys with no underscores" do
      expect(described_class.case_counterpart(:foo)).to be_nil
    end

    it "treats mixed keys as snake_case (underscore takes priority)" do
      expect(described_class.case_counterpart(:foo_barBaz)).to eq :fooBarBaz
    end
  end
end
